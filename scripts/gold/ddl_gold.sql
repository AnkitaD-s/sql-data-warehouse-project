...
CREATE OR REPLACE VIEW gold.dim_products
 AS
 SELECT row_number() OVER (ORDER BY pi.prd_start_dt, pi.prd_id) AS product_key,
   ...
   FROM silver.crm_prd_info pi
     LEFT JOIN silver.erp_px_cat_g1v2 pc ON pi.cat_id::text = pc.id::text
...
CREATE OR REPLACE VIEW gold.fact_sales
 ...
 SELECT sd.sls_ord_num AS order_number,
   ...
   FROM silver.crm_sales_details sd
     LEFT JOIN gold.dim_products dp ON sd.sls_prd_key::text = dp.product_number::text
